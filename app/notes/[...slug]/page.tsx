        ...
        <article className="reader-article">
          ...
          <header className="article-heading">
            <div className="section-kicker">
              NOTES / {article.category.toUpperCase()}
            </div>
            <h1>{article.title}</h1>
            <p>{article.summary}</p>
            <div className="article-meta">
              <span>
                <BookOpen size={15} />{' '}
                {article.isOutline ? '学习提纲' : '学习笔记'}
              </span>
              <span>
                <Clock3 size={15} /> 约 {article.minutes} 分钟
              </span>
              <a href={article.sourceUrl} target="_blank" rel="noreferrer">
                原始笔记 <ArrowUpRight size={14} />
              </a>
            </div>
          </header>
          {article.isOutline && (
            <p className="outline-notice">
              这篇笔记目前是学习提纲，保留原有内容。
            </p>
          )}
        ...
        {toc.length > 0 && (
          <aside className="article-toc">
            <nav aria-label="本文目录">
              ...
              <ol>
                {toc.map((h) => (
                  <li key={h.id} className={h.level === 3 ? 'toc-nested' : ''}>
